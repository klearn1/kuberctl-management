...
func TestValidateVolumesForPVCs(t *testing.T) {
	...
	if len(v1err) == 0 {
		t.Errorf("Invalid test volumes - expected failed %v", field.Invalid(fldPath.Child("persistentVolumeClaim").Child("claimName"), "testclaim1",
			"must not reference one PVC multi times in volumes"))
		return
	}

	if v1err[0].Type != field.ErrorTypeInvalid {
		t.Errorf("unexpected error type: got %v, want %v", v1err[0].Type, field.ErrorTypeInvalid)
	}
	if !strings.Contains(v1err[0].Detail, "must not reference one PVC multi times in volumes") {
		t.Errorf("unexpected error detail: got %v, want %v", v1err[0].Detail, "must not reference one PVC multi times in volumes")
	}
}
