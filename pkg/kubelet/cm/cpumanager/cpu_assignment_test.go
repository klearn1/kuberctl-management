...
func TestTakeByTopologyUnCoreCachePacked(t *testing.T) {
	testCases := []struct {
		...
		{
			"take first available uncore cache from first socket",
		...
		{
			"take cpus from best available uncore cache group of multi uncore cache single socket - SMT disabled",
		...
		{
			"take cpus from multiple uncore cache of single socket - SMT disabled",
			topoUncoreSingleSocketSMT,
			StaticPolicyOptions{AlignByUnCoreCacheOption: true},
