...
func TestNewManagerImpl(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.description, func(t *testing.T) {
			...
			manager, err := NewManagerImpl(tCtx, kubeClient, test.stateFileDirectory, "worker")
			if test.wantErr {
				assert.Error(t, err)
				return
			}
...
func TestGetResources(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.description, func(t *testing.T) {
			tCtx := ktesting.Init(t)
			manager, err := NewManagerImpl(tCtx, kubeClient, t.TempDir(), "worker")
			assert.NoError(t, err)
			...
			containerInfo, err := manager.GetResources(tCtx, test.pod, test.container)
			if test.wantErr {
				assert.Error(t, err)
			} else {
