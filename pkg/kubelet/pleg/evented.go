...
type EventedPLEG struct {
	...
	// Structured logging for Evented PLEG
	logger klog.Logger
}
...
func NewEventedPLEG(runtime kubecontainer.Runtime, runtimeService internalapi.RuntimeService, eventChannel chan *PodLifecycleEvent,
	...
	return &EventedPLEG{
		...
		relistDuration:              relistDuration,
		clock:                       clock,
		logger:                      klog.LoggerWithName(klog.Background(), "EventedPLEG"),
	}, nil
}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				e.logger.V(4).Info("Failed to get container events, retrying: ", "err", err)
			}
		}
	}()
...
func (e *EventedPLEG) processCRIEvents(containerEventsResponseCh chan *runtimeapi.ContainerEventResponse) {
	for event := range containerEventsResponseCh {
		...
		if event.PodSandboxStatus == nil || event.PodSandboxStatus.Metadata == nil {
			e.logger.Error(nil, "Received ContainerEventResponse with nil PodSandboxStatus or PodSandboxStatus.Metadata", "containerEventResponse", event)
			continue
		}
		...
		if err != nil {
			...
			if e.logger.V(6).Enabled() {
				e.logger.Error(err, "Error generating pod status from the received event", "podUID", podID, "podStatus", status)
			} else {
				e.logger.Error(err, "Error generating pod status from the received event", "podUID", podID)
			}
		} else {
			if klogV := e.logger.V(6); klogV.Enabled() {
				e.logger.Info("Generated pod status from the received event", "podUID", podID, "podStatus", status)
			} else {
				e.logger.V(4).Info("Generated pod status from the received event", "podUID", podID)
			}
...
func (e *EventedPLEG) sendPodLifecycleEvent(event *PodLifecycleEvent) {
	...
	default:
		...
		e.logger.Error(nil, "Event channel is full, discarded pod lifecycle event")
	}
}
...
func (e *EventedPLEG) updateRunningPodMetric(podStatus *kubecontainer.PodStatus) {
	...
	if err != nil {
		e.logger.Error(err, "Get cache", "podID", podStatus.ID)
	}
