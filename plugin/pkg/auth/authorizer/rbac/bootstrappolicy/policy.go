...
func ClusterRoles() []rbacv1.ClusterRole {
	...
	kubeltAPIAdminRules := []rbacv1.PolicyRule{
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.KubeletFineGrainedAuthz) {
		kubeltAPIAdminRules = append(kubeltAPIAdminRules, rbacv1helpers.NewRule("*").Groups(legacyGroup).Resources("nodes/proxy", "nodes/metrics", "nodes/stats", "nodes/log", "nodes/pods", "nodes/healthz").RuleOrDie())
	} else {
		kubeltAPIAdminRules = append(kubeltAPIAdminRules, rbacv1helpers.NewRule("*").Groups(legacyGroup).Resources("nodes/proxy", "nodes/metrics", "nodes/stats", "nodes/log").RuleOrDie())
	}

	roles = append(roles, rbacv1.ClusterRole{
		// a role to use for full access to the kubelet API
		ObjectMeta: metav1.ObjectMeta{Name: "system:kubelet-api-admin"},
		Rules:      kubeltAPIAdminRules,
	})
