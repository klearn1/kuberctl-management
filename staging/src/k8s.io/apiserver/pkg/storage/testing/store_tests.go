		...
		go func(resourceVersion string) {
			...
			if perNode {
				for _, nodeName := range nodeNames {
					...
					paginate(ctx, store, &objectCount, &pageCount, limit, resourceVersion, match, field)
				}
			} else {
				paginate(ctx, store, &objectCount, &pageCount, limit, resourceVersion, match, fields.Everything())
			}
		}(resourceVersion)
	}
	wg.Wait()
...
func paginate(ctx context.Context, store storage.Interface, objectCount, pageCount *atomic.Uint64, limit int64, resourceVersion string, match metav1.ResourceVersionMatch, fields fields.Selector) {
	listOut := &example.PodList{}
	err := store.GetList(ctx, "/pods", storage.ListOptions{
		Recursive:            true,
		ResourceVersion:      resourceVersion,
