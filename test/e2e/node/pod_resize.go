...
func waitForPodResizeActuation(c clientset.Interface, podClient *e2epod.PodClient, pod, patchedPod *v1.Pod, expectedContainers []TestContainerInfo, initialContainers []TestContainerInfo, isRollback bool, updateTimeout time.Duration) *v1.Pod {
	...
	for idx := range tests {
		...
		ginkgo.It(tc.name, func(ctx context.Context) {
			...
			patchAndVerify := func(patchString string, expectedContainers []TestContainerInfo, initialContainers []TestContainerInfo, opStr string, isRollback bool) {
				...
				resizedPod := waitForPodResizeActuation(f.ClientSet, podClient, newPod, patchedPod, expectedContainers, initialContainers, isRollback, tc.updateTimeout)
...
func doPodResizeResourceQuotaTests() {
	...
	ginkgo.It("pod-resize-resource-quota-test", func(ctx context.Context) {
		...
		resizedPod := waitForPodResizeActuation(f.ClientSet, podClient, newPod1, patchedPod, expected, containers, false, 0)
