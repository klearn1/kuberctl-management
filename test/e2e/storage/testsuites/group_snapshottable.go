...
type volumeGroupSnapshottableTest struct {
	...
	snapshots   []*storageframework.GroupSnapshotResource
	numPods     int
	numVolumes  int
}
...
func InitVolumeGroupSnapshottableTestSuite() storageframework.TestSuite {
	patterns := []storageframework.TestPattern{
		storageframework.GroupVolumeSnapshotDelete,
	}
...
func InitCustomGroupSnapshottableTestSuite(patterns []storageframework.TestPattern) storageframework.TestSuite {
	return &VolumeGroupSnapshottableTestSuite{
		tsInfo: storageframework.TestSuiteInfo{
			...
			TestTags: []interface{}{feature.GroupSnapshotDataSource},
		},
	}
}
	...
	ginkgo.Describe("VolumeGroupSnapshottable", func() {
		...
		ginkgo.Context("", func() {
			...
			cleanup := func(ctx context.Context) {
				// for _, snapshot := range groupTest.snapshots {
				// 	framework.Logf("Deleting snapshot %s", snapshot.Vgs.GetName())
			...
			ginkgo.It("should create snapshots for multiple volumes in a pod", func(ctx context.Context) {
				...
				snapshot := storageframework.CreateGroupSnapshotResource(ctx, snapshottableDriver, groupTest.config, pattern, labelValue, groupTest.volumeGroup[0][0].Pvc.GetNamespace(), f.Timeouts, nil)
				...
				for _, volume := range volumeList {
					...
					framework.ExpectNoError(e2epod.WaitTimeoutForPodRunningInNamespace(ctx, cs, pod.Name, pod.Namespace, f.Timeouts.PodStartSlow))
				}
